...
return $result
 ...
 #Start-CAPolicyBackup -BackupFileName C:\temp\newBackup5.json
 #$Ret = Start-CAPolicyRestore -PolicyBackupFile C:\temp\newBackup3.json
 ...
 #New-CAPolicy -DisplayName "Jeevan3"  -State Disabled 
  # $result= Get-CAPolicy -Id  "128535ce-cafd-44cd-9ee2-ab12c8137cb1"  -Type PolicyID | Set-CAPolicy
  #$result = set-CApolicy -Id  "128535ce-cafd-44cd-9ee2-ab12c8137cb1"
  #$result
  #$result = Set-CAPolicy -Id "ASdsasadsdas"
 
 # Remove from CMDLets
 #result= Remove-CAPolicy -PolicyId 
 
 #$Policy.conditions.applications.excludeApplications = @("app1", "apps2")
 ##$policy | ConvertTo-Json -Depth 10 | Out-File C:\temp\otf.json

 #Get-Help Get-CAPolicy
